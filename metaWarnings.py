...
class ReqStrSugRepr(type):
    def __init__(cls,name,bases,attrd):
        super(ReqStrSugRepr,cls).__init__(name,bases,attrd) #attributes

        if '__str__' not in attrd:
            raise TypeError("Class suggests overriding of __str__()")

        if '__repr__' not in attrd:
            warn('Class sugests overriding of __repr__()\n',stacklevel=3)
